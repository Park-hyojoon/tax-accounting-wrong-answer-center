(function(){
  ...
  function render(kind){
    ...
    if(!groups.length){const p=document.createElement('p');p.textContent='직접 제출 기록에서 3회 이상 확인된 묶음이 아직 없습니다.';grid.append(p);return}
    for(const group of groups){const card=document.createElement('article');card.className='weak-card';const h=document.createElement('h3');h.textContent=group.label;card.append(h);
  ...
  for(const [kind,label] of [['same','같은 문제 3회 이상'],['type','유사 유형 3회 이상'],['concept','큰 개념 3회 이상']]){const b=document.createElement('button');b.type='button';b.dataset.kind=kind;b.textContent=label+' · '+data.groups.filter(g=>g.kind===kind).length;b.onclick=()=>render(kind);bar.append(b)}
  render('type');
})();
