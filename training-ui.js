(function(){
  ...
  let menuOrder=completeOrder(initialSettings.menuOrder,availableIds.filter(id=>!hiddenMenuIds.has(id)));
  links.forEach(link=>{link.hidden=hiddenMenuIds.has(link.dataset.menuId)});
  links.sort((a,b)=>{const ai=menuOrder.indexOf(a.dataset.menuId),bi=menuOrder.indexOf(b.dataset.menuId);return (ai<0?999:ai)-(bi<0?999:bi)});
  ...
  style.textContent=`
    ...
    .menu-settings-body{padding:17px 20px}.menu-settings-help{margin:0 0 13px;color:#647287;line-height:1.55}.menu-list-title{display:flex;align-items:center;justify-content:space-between;margin:15px 0 8px;font-size:.94rem}.menu-list-title:first-of-type{margin-top:0}.menu-list-count{color:#1877d2}.menu-order-list,.menu-waiting-list{display:grid;gap:7px;margin:0;padding:0;list-style:none}.menu-order-item{display:grid;grid-template-columns:34px 1fr auto 28px;gap:8px;align-items:center;padding:8px;border:1px solid #dce3ea;border-radius:11px;background:#f8fafc;cursor:grab}.menu-order-item:active{cursor:grabbing}.menu-order-item.dragging{opacity:.45;border-color:#1877d2}.menu-order-number{text-align:center;color:#728095;font-weight:800}.menu-order-label{font-weight:900}.menu-drag-handle{color:#8b98a8;font-size:21px;text-align:center;line-height:1}.menu-delete,.menu-restore{min-height:34px;padding:5px 10px;border:1px solid #d5dce4;border-radius:8px;background:#fff;font:inherit;font-weight:800;cursor:pointer}.menu-delete{color:#c23838}.menu-delete:hover{background:#fff0f0}.menu-waiting{display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center;padding:9px 10px;border:1px dashed #cbd5df;border-radius:10px;background:#fff}.menu-waiting-empty{padding:15px;text-align:center;color:#7b8796;border:1px dashed #d8dfe7;border-radius:10px}.menu-restore{color:#1768aa}.menu-restore:hover{background:#edf7ff}
    ...
    @media ${MOBILE}{
      ...
      .menu-order-item{grid-template-columns:28px 1fr auto 25px}.menu-settings-body{padding:14px}.menu-settings-actions{padding:12px 14px;flex-wrap:wrap}.menu-settings-actions button{flex:1}
  ...
  function renderOrder(){orderList.replaceChildren();draft.forEach((id,index)=>{const link=links.find(item=>item.dataset.menuId===id),item=document.createElement('li');item.className='menu-order-item';item.dataset.menuId=id;item.draggable=true;item.innerHTML=`<span class="menu-order-number">${index+1}</span><span class="menu-order-label"></span><button class="menu-delete" type="button" aria-label="${link?.textContent.trim()||id} 메뉴 삭제">삭제</button><span class="menu-drag-handle" aria-hidden="true">☰</span>`;item.querySelector('.menu-order-label').textContent=link?.textContent.trim()||id;orderList.append(item)});waitingList.replaceChildren();draftHidden.forEach(id=>{const link=links.find(item=>item.dataset.menuId===id),item=document.createElement('li');item.className='menu-waiting';item.dataset.menuId=id;item.innerHTML='<span class="menu-order-label"></span><button class="menu-restore" type="button">불러오기</button>';item.querySelector('.menu-order-label').textContent=link?.textContent.trim()||id;waitingList.append(item)});if(!draftHidden.length){const empty=document.createElement('li');empty.className='menu-waiting-empty';empty.textContent='대기 중인 메뉴가 없습니다.';waitingList.append(empty)}settings.querySelector('.menu-active-count').textContent=draft.length;settings.querySelector('.menu-waiting-count').textContent=draftHidden.length}
  function openSettings(){draft=[...menuOrder];draftHidden=availableIds.filter(id=>hiddenMenuIds.has(id));renderOrder();if(typeof settings.showModal==='function')settings.showModal();else settings.setAttribute('open','')}
  function closeSettings(){if(typeof settings.close==='function')settings.close();else settings.removeAttribute('open')}
  settingsButton.addEventListener('click',openSettings);
  orderList.addEventListener('click',event=>{const button=event.target.closest('.menu-delete');if(!button)return;const id=button.closest('.menu-order-item').dataset.menuId;draft=draft.filter(value=>value!==id);draftHidden.push(id);renderOrder()});
  waitingList.addEventListener('click',event=>{const button=event.target.closest('.menu-restore');if(!button)return;const id=button.closest('.menu-waiting').dataset.menuId;draftHidden=draftHidden.filter(value=>value!==id);draft.push(id);renderOrder()});
  let draggedItem=null;orderList.addEventListener('dragstart',event=>{draggedItem=event.target.closest('.menu-order-item');if(!draggedItem)return;draggedItem.classList.add('dragging');event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',draggedItem.dataset.menuId)});orderList.addEventListener('dragover',event=>{if(!draggedItem)return;event.preventDefault();const target=event.target.closest('.menu-order-item');if(!target||target===draggedItem)return;const after=event.clientY>target.getBoundingClientRect().top+target.offsetHeight/2;orderList.insertBefore(draggedItem,after?target.nextSibling:target)});orderList.addEventListener('dragend',()=>{if(!draggedItem)return;draggedItem.classList.remove('dragging');draft=[...orderList.children].map(item=>item.dataset.menuId);draggedItem=null;renderOrder()});
  settings.querySelector('.menu-default').addEventListener('click',()=>{draft=completeOrder(MENU_DEFAULT,availableIds);draftHidden=[];renderOrder()});
  ...
  settings.querySelector('.menu-save').addEventListener('click',()=>{reorderMenu(draft,draftHidden);localStorage.setItem(MENU_KEY,JSON.stringify({menuOrder,hiddenMenuIds:[...hiddenMenuIds],updatedAt:new Date().toISOString()}));closeSettings()});
  ...
  window.addEventListener('training-ui-settings',event=>{const value=event.detail||readMenuSettings();reorderMenu(value.menuOrder,value.hiddenMenuIds)});
  window.addEventListener('storage',event=>{if(event.key===MENU_KEY){const value=readMenuSettings();reorderMenu(value.menuOrder,value.hiddenMenuIds)}});
