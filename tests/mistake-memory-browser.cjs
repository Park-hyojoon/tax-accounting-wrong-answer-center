...
(async()=>{
  ...
  try{
    ...
    const after=await evaluate(`({total:TrainingMistakeMemory.analyze().total,rows:document.querySelectorAll('#frequentList .top-type').length,firstLinks:document.querySelector('.top-type-body .top-links').children.length,backup:TrainingGitHub.backupPayload().sourceMistakes.entries.length})`);
    assert.equal(after.total,initial.total);assert.equal(after.rows,5);assert.equal(after.firstLinks,0);assert.equal(after.backup,initial.total);
